...
ggplot(data=early, aes(x=update, y=coop, group=treatment, colour=treatment)) + ylab("Mean Lysogeny Rate") + xlab("Updates") + stat_summary(aes(color=treatment, fill=treatment),fun.data="mean_lysogeny_rate", geom=c("smooth"), se=TRUE) + theme(panel.background = element_rect(fill='white', colour='black')) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + guides(fill=FALSE) +scale_colour_manual(values=colors) + scale_fill_manual(values=colors) +xlim(0,20)
